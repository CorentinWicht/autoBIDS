...
%% Parameters prompts
clear variables;close all;clc % Clear workspace, etc
...
if ~isempty(mrk_folder) % useless if not provided
...
Formats(Idx+1,1).items = {'Autogenerated' 'EEGLAB' 'Load excel file'};
...
if ~isempty(RunPattern); RunLoc = fastif(strcmp(PImp.RunLoc,'files'),1,2); end
SessionPattern = PImp.SessionPattern; 
if ~isempty(SessionPattern); SessionLoc = fastif(strcmp(PImp.SessionLoc,'files'),1,2); end
if strcmp(PImp.MetaData,'Autogenerated')
    MetaData = 1;
elseif strcmp(PImp.MetaData,'EEGLAB')
    MetaData = 2;
elseif strcmp(PImp.MetaData,'Load excel file')
    MetaData = 3;
end
...
%% GENERATING/LOADING EXCEL TEMPLATE PARTICIPANT FILE
...
if MetaData == 1
    ...
    % Extracting filenames and filepaths + names
    FileNames = {FileList.name}';
    TEMP = [{FileList.folder}' repmat({'\'},length(FileList),1) {FileList.name}'];
    FileNamesPath = cell(size(TEMP,1),1);
    for k=1:size(TEMP,1); FileNamesPath{k,:} = horzcat(TEMP{k,:});
    end

    % 1. Data sheet
    if SubjLoc == 1 
        PartTab = [FileNames num2cell(SubjList)];
    elseif SubjLoc == 2
        PartTab = [FileNamesPath num2cell(SubjList)];
    end
    ...
    if length(TaskPattern)>1; TEMP = cell(size(PartTab,1),1);
        ...
        for k=1:size(PartTab,1)
            if TaskLoc == 1
                    TEMP{k} = TaskPattern{cellfun(@(x) contains(FileNames{k},x),TaskPattern)};

            elseif TaskLoc == 2
                TEMP{k} = TaskPattern{cellfun(@(x) contains(FileNamesPath{k},x),TaskPattern)};
            end
        end
    ...
    if ~isempty(RunPattern) && RunLoc == 1 
        ...
        for k=1:size(PartTab,1)
            TEMP{k} = str2double(regexp(FileNames{k}, StrPattern, 'match'));
        end; PartTab = [PartTab TEMP];
    elseif ~isempty(RunPattern) && RunLoc == 2 
        TEMP = cell(size(PartTab,1),1);
        ColNames = [ColNames {'Run'}];
        for k=1:size(PartTab,1)
            TEMP{k} = str2double(regexp(FileNamesPath{k}, StrPattern, 'match'));
        end; PartTab = [PartTab TEMP];
    end
    ...
    ToDisp = [ColNames' cell(length(ColNames),9)]; % Matrix to integrate in the following uitable
        ...
        if nnz(Idx)
            try % if cell array of strings
                ...
                ToDisp(Idx,2:2+length(unique(cell2mat(PartTab(:,Idx))))-1) = ...
                    unique(cellfun(@(x) num2str(x),PartTab(:,ismember(ColNames,Cat{k})),'UniformOutput',0))';
            end
        end
    end
...
elseif MetaData == 3
    ...
    % Ui to retrieve the file
    [File, Path] = uigetfile('*.xlsx','Select the excel file to load');
    
    % Load data
    PartInfoData = readtable([Path File],'Sheet','DATA');
    PartInfoColNames = PartInfoData.Properties.VariableNames;
    PartInfoData = table2cell(PartInfoData);
    PartInfoDesc = readtable([Path File],'Sheet','DESC');
    PartInfoDesc = table2cell(PartInfoDesc);
end
...
if strcmp(LoadSTUDY,'NO')
    ...
    for i=1:size(FileList,1)
        ...
        if MetaData==1 
            Idx = contains(cellfun(@(x) strrep(x,'.bdf',''),PartInfoData(:,1),'UniformOutput',0),...
            ...
            for m=1:size(PartInfoDesc,1)
                ...
                if length(IdxPartInfoDesc)>2
                    ...
                    catch % will issue an error if numbers
                        Levels = unique(cellfun(@(x) num2str(x),PartInfoData(:,m),'UniformOutput',0)); % find unique levels in data
                        Levels = Levels(cellfun(@(x) ~isempty(x),Levels)); % Removing empty fields
                        % replacing spaces (if any, not accepted for structure field names)
                        Levels = cellfun(@(x) strrep(x,' ','_'), Levels, 'UniformOutput', 0);
                        
                        % transform numbers to written form
                        Levels = cellfun(@(x) num2words(str2double(x), 'case','sentence'), Levels,'UniformOutput',0);
                    end
...
% ----------------------------------- %
ExportPath = uigetdir(pwd,'Choose a folder where to export BIDS results');
pop_exportbids(STUDY, EEG,'targetdir',ExportPath)
